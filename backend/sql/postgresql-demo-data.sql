...
TRUNCATE TABLE public.sys_org_units,
               ...
               public.oa_expense_application,
               public.oa_expense_item,
               public.oa_holiday,
               public.oa_leave_application,
               public.oa_leave_balance,
               public.oa_leave_type,
               public.oa_outing_application,
               public.oa_overtime_application,
...
-- ----------------------------
-- 插入 sys_membership_roles 租户成员-角色关联关系
-- ----------------------------
INSERT INTO public.sys_membership_roles (id, membership_id, tenant_id, role_id, is_primary, status)
VALUES (2, 2, 1, 2, true, 'ACTIVE');
SELECT setval('sys_membership_roles_id_seq', (SELECT COALESCE(MAX(id), 1) FROM sys_membership_roles));
...
-- ----------------------------
-- oa_attendance_setting 考勤设置（每租户一行）
-- ----------------------------
...
VALUES (1, '09:00', '18:00', now(), now(), 2, 2);
SELECT setval('oa_attendance_setting_id_seq', (SELECT COALESCE(MAX(id), 1) FROM oa_attendance_setting));

-- ----------------------------
-- oa_leave_type 请假类型（ANNUAL/SICK/PERSONAL）
-- ----------------------------
INSERT INTO public.oa_leave_type (tenant_id, code, name, remark, created_at, updated_at, created_by, updated_by) VALUES
    (1, 'ANNUAL', '年假', '法定年休假', now(), now(), 2, 2),
    (1, 'SICK', '病假', '医疗期病假', now(), now(), 2, 2),
    (1, 'PERSONAL', '事假', '个人事务假', now(), now(), 2, 2);
SELECT setval('oa_leave_type_id_seq', (SELECT COALESCE(MAX(id), 1) FROM oa_leave_type));

-- ----------------------------
-- oa_holiday 2026 节假日（HOLIDAY 法定假 / WORKDAY 调休上班）
-- ----------------------------
INSERT INTO public.oa_holiday (tenant_id, date, holiday_type, name, created_at, updated_at, created_by, updated_by) VALUES
    (1, '2026-01-01', 'HOLIDAY', '元旦', now(), now(), 2, 2),
    (1, '2026-02-16', 'HOLIDAY', '春节', now(), now(), 2, 2),
    (1, '2026-02-17', 'HOLIDAY', '春节', now(), now(), 2, 2),
    (1, '2026-02-18', 'HOLIDAY', '春节', now(), now(), 2, 2),
    (1, '2026-02-15', 'WORKDAY', '春节调休上班', now(), now(), 2, 2),
    (1, '2026-02-28', 'WORKDAY', '春节调休上班', now(), now(), 2, 2),
    (1, '2026-04-04', 'HOLIDAY', '清明节', now(), now(), 2, 2),
    (1, '2026-05-01', 'HOLIDAY', '劳动节', now(), now(), 2, 2),
    (1, '2026-10-01', 'HOLIDAY', '国庆节', now(), now(), 2, 2),
    (1, '2026-10-02', 'HOLIDAY', '国庆节', now(), now(), 2, 2);
...
-- ----------------------------
-- oa_attendance_record 考勤记录（user=2，近期工作日，状态混合）
-- ----------------------------
INSERT INTO public.oa_attendance_record (tenant_id, user_id, work_date, check_in_at, check_in_latitude, check_in_longitude, check_in_wifi_bssid, check_out_at, check_out_latitude, check_out_longitude, check_out_wifi_bssid, day_result, created_at, updated_at, created_by, updated_by) VALUES
    (1, 2, '2026-08-17', '2026-08-17 08:55:00', 39.9042, 116.4074, 'AA:BB:CC:DD:00:01', '2026-08-17 18:02:00', 39.9042, 116.4074, 'AA:BB:CC:DD:00:01', 'NORMAL', now(), now(), 2, 2),
    (1, 2, '2026-08-18', '2026-08-18 10:12:00', 39.9042, 116.4074, 'AA:BB:CC:DD:00:01', '2026-08-18 18:05:00', 39.9042, 116.4074, 'AA:BB:CC:DD:00:01', 'LATE', now(), now(), 2, 2),
    (1, 2, '2026-08-19', NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, 'ABSENT', now(), now(), 2, 2),
    (1, 2, '2026-08-20', NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, 'ON_LEAVE', now(), now(), 2, 2),
    (1, 2, '2026-08-21', '2026-08-21 08:58:00', 39.9042, 116.4074, 'AA:BB:CC:DD:00:01', '2026-08-21 18:00:00', 39.9042, 116.4074, 'AA:BB:CC:DD:00:01', 'NORMAL', now(), now(), 2, 2);
SELECT setval('oa_attendance_record_id_seq', (SELECT COALESCE(MAX(id), 1) FROM oa_attendance_record));

-- ----------------------------
-- oa_leave_balance 请假额度（user=2，每类型一行，year=2026）
-- ----------------------------
INSERT INTO public.oa_leave_balance (tenant_id, user_id, leave_type_id, year, total_days, used_days, created_at, updated_at, created_by, updated_by) VALUES
    (1, 2, 1, 2026, 5, 1, now(), now(), 2, 2),
    (1, 2, 2, 2026, 10, 0, now(), now(), 2, 2),
    (1, 2, 3, 2026, 0, 0, now(), now(), 2, 2);
SELECT setval('oa_leave_balance_id_seq', (SELECT COALESCE(MAX(id), 1) FROM oa_leave_balance));

-- ----------------------------
-- oa_leave_application 请假申请（引用 leave_type；状态混合；部分挂实例）
-- ----------------------------
INSERT INTO public.oa_leave_application (tenant_id, leave_type_id, start_date, end_date, days, reason, start_half, end_half, leave_status, instance_id, created_at, updated_at, created_by, updated_by) VALUES
    (1, 1, '2026-08-20', '2026-08-20', 1, '家中有事', 0, 1, 'PENDING', 1, now(), now(), 2, 2),
    (1, 2, '2026-09-01', '2026-09-02', 2, '感冒发烧', 0, 1, 'REJECTED', NULL, now(), now(), 2, 2),
    (1, 3, '2026-10-05', '2026-10-05', 1, '办理证件', 0, 1, 'APPROVED', NULL, now(), now(), 2, 2),
    (1, 1, '2026-11-01', '2026-11-01', 1, '事假一天', 0, 1, 'WITHDRAWN', NULL, now(), now(), 2, 2);
SELECT setval('oa_leave_application_id_seq', (SELECT COALESCE(MAX(id), 1) FROM oa_leave_application));

-- ----------------------------
-- oa_expense_application 报销申请（状态混合；部分挂实例）
-- ----------------------------
INSERT INTO public.oa_expense_application (tenant_id, title, total_amount, expense_status, instance_id, created_at, updated_at, created_by, updated_by) VALUES
    (1, '差旅费报销-北京', 1500, 'APPROVED', 2, now(), now(), 2, 2),
    (1, '办公用品采购', 320, 'PENDING', NULL, now(), now(), 2, 2),
    (1, '会议费报销', 800, 'REJECTED', NULL, now(), now(), 2, 2);
SELECT setval('oa_expense_application_id_seq', (SELECT COALESCE(MAX(id), 1) FROM oa_expense_application));

-- ----------------------------
-- oa_expense_item 报销明细（引用上方报销单；每单两条）
-- ----------------------------
INSERT INTO public.oa_expense_item (tenant_id, expense_application_id, category, amount, expense_date, description, invoice_file_id, created_at, updated_at, created_by, updated_by) VALUES
    (1, 1, '交通', 800, '2026-08-15', '北京高铁往返', NULL, now(), now(), 2, 2),
    (1, 1, '住宿', 700, '2026-08-15', '北京酒店一晚', NULL, now(), now(), 2, 2),
    (1, 2, '办公用品', 320, '2026-08-16', '打印纸和墨盒', NULL, now(), now(), 2, 2),
    (1, 3, '会议', 500, '2026-08-10', '场地租赁费', NULL, now(), now(), 2, 2),
    (1, 3, '餐饮', 300, '2026-08-10', '会议工作餐', NULL, now(), now(), 2, 2);
SELECT setval('oa_expense_item_id_seq', (SELECT COALESCE(MAX(id), 1) FROM oa_expense_item));

-- ----------------------------
-- oa_business_trip_application 出差申请（状态混合）
-- ----------------------------
INSERT INTO public.oa_business_trip_application (tenant_id, title, destination, start_date, end_date, itinerary, trip_status, instance_id, created_at, updated_at, created_by, updated_by) VALUES
    (1, '北京客户拜访', '北京市朝阳区', '2026-09-10', '2026-09-11', '拜访客户A洽谈合作', 'REJECTED', 3, now(), now(), 2, 2),
    (1, '上海技术交流', '上海市浦东新区', '2026-10-01', '2026-10-02', '参加技术峰会', 'PENDING', NULL, now(), now(), 2, 2),
    (1, '广州分公司支持', '广州市天河区', '2026-11-05', '2026-11-06', '协助分公司系统上线', 'APPROVED', NULL, now(), now(), 2, 2);
SELECT setval('oa_business_trip_application_id_seq', (SELECT COALESCE(MAX(id), 1) FROM oa_business_trip_application));

-- ----------------------------
-- oa_outing_application 外出申请（状态混合）
-- ----------------------------
INSERT INTO public.oa_outing_application (tenant_id, reason, destination, start_time, end_time, outing_status, instance_id, created_at, updated_at, created_by, updated_by) VALUES
    (1, '拜访客户', '北京市海淀区中关村', '2026-08-22 09:00:00', '2026-08-22 12:00:00', 'PENDING', 4, now(), now(), 2, 2),
    (1, '参加会议', '上海市徐汇区某会议中心', '2026-09-05 13:00:00', '2026-09-05 18:00:00', 'REJECTED', NULL, now(), now(), 2, 2),
    (1, '现场技术支持', '深圳市南山区科技园', '2026-10-10 09:00:00', '2026-10-10 17:00:00', 'APPROVED', NULL, now(), now(), 2, 2);
SELECT setval('oa_outing_application_id_seq', (SELECT COALESCE(MAX(id), 1) FROM oa_outing_application));

-- ----------------------------
-- oa_overtime_application 加班申请（状态混合；compensation_type 混合）
-- ----------------------------
INSERT INTO public.oa_overtime_application (tenant_id, reason, start_time, end_time, compensation_type, overtime_status, instance_id, created_at, updated_at, created_by, updated_by) VALUES
    (1, '版本发布前紧急修复', '2026-08-22 18:30:00', '2026-08-22 22:00:00', 'COMP_LEAVE', 'PENDING', NULL, now(), now(), 2, 2),
    (1, '系统迁移', '2026-09-01 18:00:00', '2026-09-01 23:00:00', 'OVERTIME_PAY', 'APPROVED', NULL, now(), now(), 2, 2),
    (1, '客户故障处理', '2026-10-01 18:00:00', '2026-10-01 20:00:00', 'COMP_LEAVE', 'REJECTED', NULL, now(), now(), 2, 2);
SELECT setval('oa_overtime_application_id_seq', (SELECT COALESCE(MAX(id), 1) FROM oa_overtime_application));

-- ----------------------------
-- oa_seal_application 用印申请（状态混合；seal_type 混合）
-- ----------------------------
