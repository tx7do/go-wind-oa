...
//
// 单层服务：实现 kratos 生成的 WorkflowServiceHTTPServer，直接对外提供 HTTP 接口，
// 同时持有本模块的 ent 仓库与对 go-wind-cms internal_message 服务的 gRPC 客户端。
//
...
type WorkflowService struct {
	oav1.WorkflowServiceHTTPServer
	log *log.Helper
	...
	// notificationClient go-wind-cms 站内信通知组件的 gRPC 客户端。
	// 审批流转时据此异步通知下一审批人 / 申请人，复用既有消息通道，不在 OA 内重造。
	notificationClient internalMessageV1.InternalMessageServiceClient
}
...
func NewWorkflowService(
	...
	return &WorkflowService{
		log:                ctx.NewLoggerHelper("workflow/service/oa-service"),
...
// notifyAsync 经 go-wind-cms 站内信通道异步通知某用户。
...
func (s *WorkflowService) notifyAsync(ctx context.Context, recipientUserId uint32, title, content string) {
	if recipientUserId == 0 || s.notificationClient == nil {
		return
	}
	go func() {
		...
		defer cancel()
		_, _ = s.notificationClient.SendMessage(notifyCtx, &internalMessageV1.SendMessageRequest{
...
// 需对接 go-wind-cms identity 服务，留作后续扩展。
