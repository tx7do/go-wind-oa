...
func (_q *WorkflowTaskQuery) FirstID(ctx context.Context) (id uint32, err error) {
	var ids []uint32
...
func (_q *WorkflowTaskQuery) FirstIDX(ctx context.Context) uint32 {
...
func (_q *WorkflowTaskQuery) OnlyID(ctx context.Context) (id uint32, err error) {
	var ids []uint32
...
func (_q *WorkflowTaskQuery) OnlyIDX(ctx context.Context) uint32 {
...
func (_q *WorkflowTaskQuery) IDs(ctx context.Context) (ids []uint32, err error) {
...
func (_q *WorkflowTaskQuery) IDsX(ctx context.Context) []uint32 {
	ids, err := _q.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
//		GroupBy(workflowtask.FieldCreatedAt).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.WorkflowTask.Query().
//		Select(workflowtask.FieldCreatedAt).
//		Scan(ctx, &v)
...
func (_q *WorkflowTaskQuery) prepareQuery(ctx context.Context) error {
	...
	if workflowtask.Policy == nil {
		return errors.New("ent: uninitialized workflowtask.Policy (forgotten import ent/runtime?)")
	}
	if err := workflowtask.Policy.EvalQuery(ctx, _q); err != nil {
		return err
	}
	return nil
}
...
func (_q *WorkflowTaskQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*WorkflowTask, error) {
	...
	if withFKs {
		_spec.Node.Columns = append(_spec.Node.Columns, workflowtask.ForeignKeys...)
	}
...
func (_q *WorkflowTaskQuery) loadInstance(ctx context.Context, query *WorkflowInstanceQuery, nodes []*WorkflowTask, init func(*WorkflowTask), assign func(*WorkflowTask, *WorkflowInstance)) error {
	...
	nodeids := make(map[uint32][]*WorkflowTask)
	for i := range nodes {
...
func (_q *WorkflowTaskQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(workflowtask.Table, workflowtask.Columns, sqlgraph.NewFieldSpec(workflowtask.FieldID, field.TypeUint32))
	_spec.From = _q.sql
...
func (_q *WorkflowTaskQuery) ForUpdate(opts ...sql.LockOption) *WorkflowTaskQuery {
...
func (_q *WorkflowTaskQuery) ForShare(opts ...sql.LockOption) *WorkflowTaskQuery {
	if _q.driver.Dialect() == dialect.Postgres {
		_q.Unique(false)
	}
	_q.modifiers = append(_q.modifiers, func(s *sql.Selector) {
...
func (_q *WorkflowTaskQuery) Modify(modifiers ...func(s *sql.Selector)) *WorkflowTaskSelect {
...
// Modify adds a query modifier for attaching custom logic to queries.
func (_s *WorkflowTaskSelect) Modify(modifiers ...func(s *sql.Selector)) *WorkflowTaskSelect {
