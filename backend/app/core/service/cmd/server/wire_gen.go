...
func initApp(context *bootstrap.Context) (*kratos.App, func(), error) {
	...
	workflowInstanceJoinRepo := data.NewWorkflowInstanceJoinRepo(context, entClient)
	workflowInstanceParentLinkRepo := data.NewWorkflowInstanceParentLinkRepo(context, entClient)
	workflowDelegationRepo := data.NewWorkflowDelegationRepo(context, entClient)
	workflowEventRegistry := service.NewWorkflowEventRegistry()
	workflowService := service.NewWorkflowService(context, workflowDefinitionRepo, workflowInstanceRepo, workflowTaskRepo, workflowLogRepo, workflowResolverRepo, workflowInstanceJoinRepo, workflowInstanceParentLinkRepo, workflowDelegationRepo, workflowEventRegistry, internalMessageService)
	...
	geofenceRepo := data.NewGeofenceRepo(context, entClient)
	wifiFingerprintRepo := data.NewWifiFingerprintRepo(context, entClient)
	attendanceService := service.NewAttendanceService(context, attendanceRepo, leaveApplicationRepo, workflowResolverRepo, geofenceRepo, wifiFingerprintRepo)
