...
| 休暇管理 | 種別と残高、半日粒度、承認時の自動残高控除、自動ガイド付きプロセス定義 | ✅ |
| 経費精算 | 複数明細行、請求書写真の直接アップロード、自動ガイド付きプロセス定義 | ✅ |
| 社内メッセージ | 承認通知の DB 永続化、受信箱クエリ、SSE プッシュ | ✅ |
...
- `src/pages/app/oa/` — 承認センター、勤怠記録、祝祭日設定、休暇管理、経費精算、プロセス定義エディター。
...
- `lib/src/features/oa/services/` — 各業務サービス（workflow / attendance / leave / expense / file_upload / notification）。
- `lib/src/features/oa/pages/` — ログイン、承認タスクリストと詳細、申請提出（動的フォーム）、勤怠打刻、休暇、経費、通知。
