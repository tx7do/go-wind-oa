# go-wind-oa｜OA 工作流审批系统

> **轻量级、线性、状态机驱动的工作流审批引擎 — go-wind-oa**

go-wind-oa 是一个办公自动化（OA）工作流审批系统，包含轻量级工作流审批引擎、管理后台与移动端。

后端基于 GO 微服务框架 [go-kratos](https://go-kratos.dev/)，采用 core/admin/app 三 service 分離架構：core-service 为純 gRPC 后端持有工作流引擎与站內信，admin-service / app-service 为 HTTP 边端转发层。

前端 admin 为 Vue3 + Element-Plus，mobile 为 Flutter。
...
```
go-wind-oa/
├── backend/            # OA 后端（Kratos + Ent + Wire，三 service 架構）
...
│   └── mobile/         # 移動端（Flutter）
└── docs/
    ├── oa-workflow-design.md   # 後端架構
    └── oa-mobile-design.md     # 移動端架構
```
...
- **admin-service**：HTTP 边端，為 admin 前端暴露鑑權 + 站內信 + 工作流的 HTTP 端點。各 service 方法為轉發層，經 gRPC 客戶端打 core-service。中間件 `logging → auth+authz(白名單) → ent`，auth 必須在 ent 之前（順序顛倒則 ent 兜底 SystemViewer，租戶隔離失效）。
- **app-service**：HTTP 边端，為移動端暴露鑑權 + 工作流的 HTTP 端點。同款轉發層與中間件順序。
