    ...
    <ElCard shadow="never" class="flex-1">
      <ElTable :data="applications" border stripe v-loading="loadingApps">
        ...
        <ElTableColumn label="状态" width="100">
          <template #default="{ row }">
            <ElTag :type="statusTag(row.outingStatus)">{{ statusLabel(row.outingStatus) }}</ElTag>
          </template>
        </ElTableColumn>
...
import { useListOutingApplications } from "@/api/composables";
...
const appsQuery = useListOutingApplications(
  reactive({
    userId: 0,
    status: undefined,
