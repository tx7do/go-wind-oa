...
export function connectSSEServer(): void {
  ...
  // 通知流以访问令牌命名：后端 HandleAuthorize 强制 ?stream= 与令牌一致，
  // 发布侧（SendMessage）也按 GetAccessTokens 返回的令牌寻流推送。
  const token = accessStore.accessToken ?? "";
  if (!token) {
    console.warn("[TokenRefresh] No access token, skip SSE connection");
    return;
  }
  const targetSseUrl = `${import.meta.env.VITE_APP_SSE_URL}?stream=${token}`;
...
function reconnectSSEServer(): void {
  ...
  // 流 ID 必须是（刷新后的新）访问令牌，理由同 connectSSEServer。
  const token = accessStore.accessToken ?? "";
  if (!token) {
